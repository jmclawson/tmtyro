...
### Standardize titles
The `standardize_titles()` function makes it easy to convert titles to something cleaner, adopting title case, converting underscores to spaces, and dropping initial articles:
...
### N-grams
By default, the `visualize()` function returns a network visualization like that demonstrated in *Text Mining with R*:
...
The `visualize()` function returns bars showing the top words for each document. This can be a useful way to differentiate texts in a set from each other. Remember, `tfidf_dubliners` was prepared with `load_texts(remove_names = TRUE)`. The resulting chart begins to show delineation of topics characteristic of the stories in Joyce's collection:
