...
If files are already collected in a folder on disk, they can be prepared in a table by passing the name of the folder to `load_texts()`. When loaded this way, the first part of the file name populates the `doc_id` column.
...
Passing the complete path makes it possible to work with a folder of texts outside the project directory. For instance, if a collection of Shakespeare plays are kept in some other folder on my computer, I can indicate that here:

```{r}
#| eval: false
corpus_shakespeare <- load_texts("~/DH-Local/shakespeare_plays/historical", poetry = TRUE)
head(corpus_shakespeare)
```

```{r}
#| echo: false
if (interactive()) {
  corpus_shakespeare <- load_texts("~/DH-Local/shakespeare_plays/historical", poetry = TRUE) |> head(10)
  saveRDS(corpus_shakespeare, "corpus_shakespeare.rds")
} else {
  corpus_shakespeare <- readRDS("corpus_shakespeare.rds")
}
...
The process is the same when comparing the chapters or stories in a collection of works, when it might make sense to choose some other column as the document identifier. Since the table for *Dubliners* shows each story's title under "part," it makes sense to identify documents by that column:
...
This example also uses `standardize_titles()` to make the `doc_id` column look a little nicer.
...
After `add_ngrams()`, the `tabulize()` function returns the top n-grams per document. By default, the first six are shown, but rows can be chosen freely.
