...
* When it's time to share your work, a function prepares a narrative of the process that was automatically logged along the way.
...
* `summarize_tf_idf()` returns a data frame for every unique token in each document in a corpus, with columns indicating weights for term frequency-inverse document frequency.
...
But understanding context of key words with `contextualize()` might be especially helpful.
...
Alternatively, word frequencies can be shown in a document-frequency matrix using `expand_documents()`:
...
`add_vocabulary()` adds measurements of vocabulary richness, including cumulative vocabulary size and indicators of hapax legomena.
...
For a document-frequency matrix, `tabulize()` will show frequencies for the top 6 words in the corpus:
...
When used after `add_vocabulary()`, `tabulize()` prepares a clean summary table comparing many traits.
...
For sentiment analysis, `tabulize()` returns a summary of measurements for each document.
...
Before sharing a table, `italicize_titles()` adds one more step of polish:
...
For a document-frequency matrix, `visualize()` will prepare a heatmap:
...
Before sharing a figure, `italicize_titles()` adds one more step of polish:
...
When it's time to write up the methods used to find a set of conclusions, it's important to document the steps taken. `narrativize()` helps turn a logged set of steps into bullet points or a paragraph of methods:
...
Options allow for changes in format and person, but the final output will still need some manual tweaking.
