...
This option *nearly* gets to the fleeting, unsaved table above. It has a lot of empty lines and it lacks paragraph markers, but these can be corrected. The `mutate()` function of the `dplyr` package eases the process of adding a new column, as for paragraph number. And blank lines offer a good way of counting paragraphs, using the `cumsum()` function to keep a running tally:
...
The blank lines have been eradicated, but our paragraph numbers are all wrong! Using the `lag()` function from `dplyr` makes it easy to compare one cell with the value above it. Effectively, we're going to make a new running tally of every time the paragraph number changes using `cumsum()`. We could do all of this in one step, but it may make more sense to separate it out into multiple columns, which might be invisible without some scrolling:
...
This step *almost* works, but the `lag()` has nowhere to look to get a value for the first row. Lacking that, it returns an empty string. We'll need to add one more step to change the empty string into a zero. Piping `lag_num` into an `ifelse()` test lets us check to see if it's empty (`NA`) using `is.na()`; if it's empty, the test will return 0, but if it's not, it will return the value:
...
By default, `load_texts()` will load and process an entire folder of texts. But by adjusting a few options, we can convince the it to do less. We can, for instance, get it to load a single file by naming it:
...
There are other options available, too, but they deserve closer consideration than can be offered in this document.

### Summary of documents

Once loaded, this corpus of texts by Jane Austen offers many possibilities for text mining and analysis.

```{r echo=FALSE}
austen_texts |> 
  group_by(doc_id) |> 
  summarize(paragraphs = max(par_num),
            words = n()) |> 
    ungroup() |> 
  standardize_titles() |> 
  rename(title = doc_id)
